...
\textbf{Stage 2: Candidate genome filtering.} For each seed hit, we retrieve the unitig's color bitmap and accumulate per-genome vote counts. Genomes with fewer than $\min(10, 0.3 \times |\text{seeds}|)$ votes are discarded.

\textbf{Stage 3: Colinear chaining.} For each candidate genome, seed hits are mapped to genome coordinates using the genome path index. We compute the highest-scoring colinear chain using dynamic programming with a Fenwick tree (binary indexed tree) for $O(h \log h)$ time complexity, where $h$ is the number of anchors. The scoring function includes gap penalties proportional to the difference between reference and query gap lengths:
\[
...
Dragon is implemented in Rust and is available as a single binary with two subcommands: \texttt{dragon index} for index construction and \texttt{dragon search} for querying. Key dependencies include the \texttt{fm-index} crate (run-length FM-index), \texttt{roaring} (Roaring Bitmaps), \texttt{memmap2} (memory-mapped I/O), and \texttt{rayon} (parallel processing). The benchmark pipeline uses Snakemake with conda environments for reproducibility.
...
\caption{Overview of Dragon's index construction and query pipeline. The colored compacted de~Bruijn graph collapses shared sequence across genomes, the run-length FM-index provides space-efficient seed matching, and graph-aware chaining produces accurate alignments. The entire query pipeline operates within a 4~GB RAM budget via memory-mapped index access.}
